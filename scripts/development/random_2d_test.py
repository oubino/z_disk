...
d = getdistances(xy, 1000.0, verbose=False)[1]
d = (d[:, 0] ** 2 + d[:, 1] ** 2) ** 0.5
...
pdf = (4 * bg_x / (square_size**4)) * (
    (np.pi / 2) * square_size**2 - 2 * square_size * bg_x + 0.5 * bg_x**2
)
bg = pdf * bin_width * len(d)

# model the linear bit
pdf_2 = (
    (2 * np.pi * bg_x / (square_size**2) - 8 * bg_x**2 * square_size**-3)
    * bin_width
    * len(d)
)
