...
for protein in ["ACTN2", "Z1Z2", "ZASP6"]:
    ...
    input_folder = os.path.join(
        f"experiments/{protein}/output/segmented_z_disks_denoised"
    )
    ...
    plt.title(f"{protein}")
    output_loc = os.path.join(output_folder, f"figure_k1/{protein}.svg")
    ...
    table_xprec_mean.append(f"{xprec_mean} " + "\u00b1" + f" {xprec_std}")

    yprec_mean = pd_df.mean()["y"].round(1)
    yprec_std = pd_df.std(ddof=0)["y"].round(1)
    table_yprec_mean.append(f"{yprec_mean} " + "\u00b1" + f" {yprec_std}")

    zprec_mean = pd_df.mean()["z"].round(1)
    zprec_std = pd_df.std(ddof=0)["z"].round(1)
    table_zprec_mean.append(f"{zprec_mean} " + "\u00b1" + f" {zprec_std}")

    table_nzdisks.append(len(files))

    table_locsperzdisk_mean.append(
        f"{int(np.mean(locs_per_zdisk).round(0))} "
        + "\u00b1"
        + f" {int(np.std(locs_per_zdisk, ddof=0).round(0))}"
    )

    table_zdisksize_x_mean.append(
        f"{np.mean(zdisksize_x).round(1)} "
        + "\u00b1"
        + f" {np.std(zdisksize_x, ddof=0).round(1)}"
    )

    table_zdisksize_y_mean.append(
        f"{np.mean(zdisksize_y).round(1)} "
        + "\u00b1"
        + f" {np.std(zdisksize_y, ddof=0).round(1)}"
    )

    table_zdisksize_z_mean.append(
        f"{np.mean(zdisksize_z).round(1)} "
        + "\u00b1"
...
output_loc = os.path.join(output_folder, f"table_k1.csv")
