...
def main(argv=None):
    ...
    for file in files:
        ...
        df = df.filter(
            pl.col("Group Sigma X Pos").str.strip_chars_start(" ").cast(pl.Float64) < args.localisation_precision_filter
        )
        df = df.filter(
            pl.col("Group Sigma Y Pos").str.strip_chars_start(" ").cast(pl.Float64) < args.localisation_precision_filter
        )
        df = df.filter(
        ...
        d_values = getdistances(xyz_values, 
    ...
    plotting.plot_histograms(
        d_values, dims=3, filter_distance=args.filter_distance, info=info, binsize=args.bin_size,
        ) 
